(function () {
  ...
  `;
    ...
    function startPulseAnimation() {
        ...
        // Pulse every 8-12 seconds. Randomize slightly.
        const pulse = () => {
            if (!STATE.isOpen) {
                launcher.classList.add('trm-pulse');
                setTimeout(() => launcher.classList.remove('trm-pulse'), 2000); // 2s duration matches CSS
            }
        };
    ...
    function handleShowLeadForm() {
        ...
        cancelBtn.onclick = () => {
            ...
            addMessage({ text: "Message cancelled.", type: 'bot' });
            endFlow();
        };
    ...
    function renderCalendar() {
        ...
        cancelBtn.onclick = () => {
            ...
            addMessage({ text: "Booking cancelled.", type: 'bot' });
            endFlow();
        };
    ...
    function renderTimeSlots(year, month, day) {
        ...
        estHours.forEach(hour => {
            ...
            if (targetDate) {
                ...
                btn.onclick = () => {
                    handleBookingConfirm(year, month, day, localStr, targetDate);
                };
    ...
    function handleBookingConfirm(year, month, day, timeStr, fullDate) {
        addMessage({ text: `I'd like to book for ${timeStr}`, type: 'user' });

        showTypingIndicator().then(() => {
            addMessage({ text: "Perfect! I've scheduled that for you.", type: 'bot' });
            endFlow();
    ...
    async function sendChatToBackend(message, transcript, pageUrl) {
        if (!CONFIG.API_CHAT_URL) return; // Placeholder exit

        // console.log("Sending chat to backend:", { message, pageUrl });
        /* 
        try { //... 
        */
    ...
    async function sendLeadToBackend(payload) {
        if (!CONFIG.API_LEAD_URL) return; // Placeholder exit

        // console.log("Sending lead to backend:", payload);
        /*
        try {
          await fetch(CONFIG.API_LEAD_URL, {
            method: 'POST',
